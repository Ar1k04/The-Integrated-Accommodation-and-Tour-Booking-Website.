...
class _ValidateBody:
    def __init__(self, code: str, booking_amount: float):
        self.code = code
...
async def validate_promo_code(
    ...
    if booking_amount < float(promo.min_booking_amount):
        return PromoCodeValidateResponse(
            valid=False,
            message=f"Minimum booking amount is ${promo.min_booking_amount}",
        )
...
async def list_promo_codes(
    ...
    return {
        "items": [PromoCodeResponse.model_validate(p).model_dump() for p in items],
        "meta": {
            "total": total,
            "page": page,
            "per_page": per_page,
