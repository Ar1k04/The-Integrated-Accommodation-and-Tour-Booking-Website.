...
export default function AdminLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-surface">
      <aside className="hidden md:flex w-64 flex-col bg-primary text-white">
        ...
        <div className="px-3 py-4 border-t border-white/10">
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            <div className="relative w-8 h-8 shrink-0">
              ...
              {isSuperAdmin && (
                <Crown className="absolute -top-2 -right-1 w-3.5 h-3.5 text-yellow-300" />
              )}
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="md:hidden flex overflow-x-auto bg-white border-b px-2 py-2 gap-1">
          {sidebarLinks.map((link) => {
            const Icon = link.icon
            const isActive = location.pathname === link.to
            return (
