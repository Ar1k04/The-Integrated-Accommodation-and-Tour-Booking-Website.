...
export default function RegisterPage() {
  ...
  const validate = () => {
    const errs = {}
    if (!form.full_name.trim()) errs.full_name = t('register.validation.fullNameRequired')
    if (!isValidEmail(form.email)) errs.email = t('register.validation.emailRequired')
    ...
    if (!agreed) errs.agreed = t('register.validation.agreeRequired')
    setErrors(errs)
  ...
  const validateAdminForm = () => {
    const errs = {}
    if (!adminForm.full_name.trim()) errs.full_name = t('register.validation.fullNameRequired')
    if (!isValidEmail(adminForm.email)) errs.email = t('register.validation.emailRequired')
    if (!isStrongPassword(adminForm.password)) errs.password = t('register.validation.passwordWeak')
    if (adminForm.password !== adminForm.confirmPassword) errs.confirmPassword = t('register.validation.passwordMismatch')
    setAdminErrors(errs)
    return Object.keys(errs).length === 0
  }
  ...
  return (
    <>
      <Helmet><title>{t('register.title')} — TravelBooking</title></Helmet>
      <div className="min-h-[80vh] flex items-center justify-center px-4 py-12 bg-surface">
        <div className="w-full max-w-md space-y-5">
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <h1 className="font-heading text-2xl font-bold text-center mb-1">{t('register.title')}</h1>
            <p className="text-gray-500 text-center text-sm mb-8">{t('register.subtitle')}</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <label className="flex items-start gap-2">
                ...
                <span className="text-sm text-gray-600">
                  ...
                  <a href="#" className="text-primary font-medium">{t('register.termsOfService')}</a>{' '}
                  {t('register.and')}{' '}
                  <a href="#" className="text-primary font-medium">{t('register.privacyPolicy')}</a>
                </span>
              </label>
            ...
            {showAdminRegister && (
              <div className="px-8 pb-8 pt-2">
                ...
                <form onSubmit={handleAdminRegister} className="space-y-5">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder={t('register.partnerName')}
                      />
                    </div>
                  ...
                  >
                    {loading ? t('register.creatingPartner') : t('register.createPartner')}
                  </button>
                </form>
              </div>
            )}
